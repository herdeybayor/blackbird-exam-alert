...
import { Faculty, FacultyHall, TimeTable, TimeTableCourse } from "@/app/generated/prisma";
...
export function FacultyManageClient({ faculties: initialFaculties }: { faculties: FacultyWithRelations[] }) {
    ...
    const [selectedFacultyForTimetable, setSelectedFacultyForTimetable] = useState("");
    const [newTimetableName, setNewTimetableName] = useState("");
    const [newTimetableSession, setNewTimetableSession] = useState("");
    
    // Course form state
    const [selectedTimetableForCourse, setSelectedTimetableForCourse] = useState("");
    ...
    const handleDeleteFaculty = async (id: string) => {
        if (!confirm("Are you sure you want to delete this faculty? This will also delete all associated halls and timetables.")) {
            return;
        }
    ...
    const handleAddHall = async () => {
        ...
        setLoading(true);
        const result = await addFacultyHall(selectedFacultyForHall, newHallName.trim(), capacity);
    ...
    const handleCreateTimetable = async () => {
        if (!selectedFacultyForTimetable || !newTimetableName.trim() || !newTimetableSession.trim()) {
            toast.error("Please fill all fields");
            return;
        }

        setLoading(true);
        const result = await createTimeTable(selectedFacultyForTimetable, newTimetableName.trim(), newTimetableSession.trim());
    ...
    const handleDeleteTimetable = async (id: string) => {
        if (!confirm("Are you sure you want to delete this timetable? This will also delete all associated courses.")) {
            return;
        }
    ...
    const handleAddCourse = async () => {
        if (!selectedTimetableForCourse || !newCourseCode.trim() || !newCourseTitle.trim() || !newCourseUnit || !newCourseStudents) {
            toast.error("Please fill all fields");
            return;
        }
        ...
        if (isNaN(unit) || unit <= 0 || isNaN(students) || students <= 0) {
            toast.error("Please enter valid numbers for course unit and number of students");
            return;
        }
                                        ...
                                        <CardContent className="flex items-center justify-between py-4">
                                            <div>
                                                ...
                                                <p className="text-sm text-muted-foreground">
                                                    {faculty.facultyHalls.length} halls • {faculty.timeTables.length} timetables
                                                </p>
                                            </div>
                                            <Button
                                                variant="destructive"
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    <div>
                                        ...
                                        <Select value={selectedFacultyForHall} onValueChange={setSelectedFacultyForHall}>
                                            <SelectTrigger>
                                    ...
                                    <div className="flex items-end">
                                        <Button onClick={handleAddHall} disabled={loading} className="w-full">
                        ...
                        <div className="space-y-4">
                            ...
                            {faculties.map((faculty) => (
                                faculty.facultyHalls.length > 0 && (
                                    <div key={faculty.id}>
                                        <h3 className="text-lg font-medium mb-3">{faculty.name}</h3>
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <div>
                                        ...
                                        <Select value={selectedFacultyForTimetable} onValueChange={setSelectedFacultyForTimetable}>
                                            <SelectTrigger>
                                                <SelectValue placeholder="Select faculty" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                ...
                                                <Card key={timetable.id}>
                                                    <CardContent className="flex items-center justify-between py-4">
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-muted-foreground">
                                                                Session: {timetable.session} • {timetable.courses.length} courses
                                                            </p>
                                                        </div>
                                                        <Button
                                                            variant="destructive"
                            ...
                            <CardContent className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                                    <div>
                                        ...
                                        <Select value={selectedTimetableForCourse} onValueChange={setSelectedTimetableForCourse}>
                                            <SelectTrigger>
                                            ...
                                            <SelectContent>
                                                {allTimetables.map((timetable) => (
                                                    <SelectItem key={timetable.id} value={timetable.id}>
                                                        {timetable.facultyName} - {timetable.name} ({timetable.session})
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                        </Select>
                                    ...
                                    timetable.courses.length > 0 && (
                                        <div key={timetable.id}>
                                            <h3 className="text-lg font-medium mb-3">
                                                {faculty.name} - {timetable.name} ({timetable.session})
                                            </h3>
                                            <div className="grid gap-3 mb-6">
                                                {timetable.courses.map((course) => (
                                                    <Card key={course.id}>
                                                        <CardContent className="flex items-center justify-between py-4">
                                                            <div>
                                                                <h4 className="font-semibold">{course.courseCode} - {course.courseTitle}</h4>
                                                                <p className="text-sm text-muted-foreground">
                                                                    Unit: {course.courseUnit} • Students: {course.numberOfStudents}
                                                                </p>
                                                            </div>
                                                            <Button
                                                                variant="destructive"
